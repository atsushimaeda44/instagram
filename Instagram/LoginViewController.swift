...
class LoginViewController: UIViewController {
    ...
    @IBAction func handleCreateAcountButton(_ sender: Any) {
        if let address = mailAdressTextField.text, let password = passwordTextField.text, let displeyName = displayNameTextField.text{
            //この中のいずれでも入力されていない場合は表示しない
            if address.isEmpty || password.isEmpty || displeyName.isEmpty{
            ...
            Auth.auth().createUser(withEmail: address, password: password) { user, error in
                if let error = error{
                    //エラーがあったら原因をプリントして、returnすることで以降の処理を実行せずに終了する
                    print("DEBUG_PRINT: " + error.localizedDescription)
                ...
                if let user = user {
                    ...
                    changeRequest.displayName = displeyName
                    changeRequest.commitChanges { error in
                        if let error = error {
                            SVProgressHUD.showError(withStatus: "ユーザー作成時にエラーが発生しました。")
                            print("DEBUG_PRINT: " + error.localizedDescription)
                        }
                        print("DEBUG_PRINT: [displayName = \(String(describing: user.displayName))]の設定成功しました。")
